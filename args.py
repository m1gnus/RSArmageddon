...
from utils import compute_d, complete_privkey, compute_pubkey
...
pem_parser.add_argument("--createpub", action="store_true", dest="cpub", help="Create a public key file in the specified format (--file-format) from numeric values")
pem_parser.add_argument("--createpriv", action="store_true", dest="cpriv", help="Create a private key file in the specified format (--file-format) from numeric values")
pem_parser.add_argument("--generate", action="store_true", dest="generatekeypair", help="Generate a new key pair")
pem_parser.add_argument("--file-format", action="store", dest="format", type=validate_file_format, default="pem", help="Specify the key file format, choose one from the follows: [PEM, DER, OpenSSH] (default: PEM)")
pem_parser.add_argument("--output-priv", action="store", dest="opriv", type=Path, default=None, help="Specify where to save the private key file created with the given private key values if --createpriv flag is setted")
pem_parser.add_argument("--output-pub", action="store", dest="opub", type=Path, default=None, help="Specify where to save the public key file created with the given public key values if --createpub flag is setted")


def _finalize_pem_args(args):
    if (args.cpub or args.cpriv) and args.generatekeypair:
        raise ValueError(f"--createpub or --createpriv cannot be specified together with --generate")
    if args.cpub and any(x is not None for x in (args.n, args.e, args.d, args.p, args.q)):
        args.n, args.e = compute_pubkey(args.n, args.e, args.d, args.p, args.q)
    if args.cpriv:
        args.n, args.e, args.d, args.p, args.q = complete_privkey(args.n, args.e, args.d, args.p, args.q)
    if args.dumpvalues and args.key_path is None:
        raise ValueError(f"--dumpvalues requires --key")
